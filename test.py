...
sLength=10000
NbClasse=5
df_anatomy=pd.read_csv('Anatomy.csv',names=["Code","Nom","Categorie"],sep="\t") 
df_Chemical=pd.read_csv('Chemicals_and_drugs.csv',names=["Code","Nom","Categorie"],sep="\t") 
df_Devices=pd.read_csv('Devices.csv',names=["Code","Nom","Categorie"],sep="\t") 
df_sign=pd.read_csv('Sign_or_Symptom.csv',names=["Code","Nom","Categorie"],sep="\t") 
df_Concept=pd.read_csv('Concept_and_Ideas.csv',names=["Code","Nom","Categorie"],sep="\t") 
df_anatomy_first=df_anatomy[0:sLength]
df_sign_first=df_sign[sLength:2*sLength]
df_Chemical_first=df_Chemical[2*sLength:3*sLength]
df_Devices_first=df_Devices[4*sLength:5*sLength]
...
df=pd.concat([df_anatomy_first,df_Chemical_first,df_Devices_first,df_sign_first,df_Concept_first])
...
features =tfidf.fit_transform(df.Nom)
	...
	for i in range(0,NbClasse*sLength):
		if df.Origine[i]==df.Origine[indicecalcul]:
			...
			if distance<minimal and indicecalcul!=i:
				print("I dont wanna wake up")
				minimal=distance
				indicetrouve=i
	result=df.Code[indicecalcul]==df.Code[indicetrouve]
	liste.append(result)
	if result==False:
		print(df.Nom[indicetrouve])
		print(df.Nom[indicecalcul])
		print(df.Code[indicetrouve])
		print(df.Code[indicecalcul])
